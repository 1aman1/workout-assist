...
## Version 1.109 (2026-08-22)
- Change summary:
  - Added a new `AppFormatters.applyMissCrashDepth(entries)` pure transform and a **"Falling miss gaps"** toggle under Settings > Streak graph. When enabled, a run of missed days no longer flatlines at `0`: the first miss after a streak still shows `0` (the "crash from the peak"), but each further consecutive miss drops one unit lower (`0, -1, -2, ...`), like a stock price falling further each day it doesn't recover. A `PENDING` (today, unlogged) entry continues at the current miss depth rather than jumping back toward zero. A new run after any gap always restarts the climb at `1` — the transform resets its internal depth counter to `0` on every `DONE` entry, so it never "recovers" back up through the negative numbers first.
  - The toggle applies to both chart looks (line and stock-candle). `MomentumYAxis`/`momentumYTicks` were generalized to take a `minValue` (in addition to `maxValue`) so the Y-axis can render negative ticks, and `MomentumCandleChart`/`MomentumLineChart` were generalized to scale against the full `[minValue, maxValue]` range instead of assuming a `0` floor. Both charts now draw a faint zero-baseline reference line whenever the visible range dips below zero. `MomentumCandleChart`'s classic "continuing miss pinned near zero" candle-sizing hack (`maxOf(open, 1)` / `0`) is now only applied when crash mode is off; with it on, candle bodies just use the plain `open`/`close` span, which already produces the right visual (a big drop from the peak to `0`, then small ticks between each deeper negative day).
  - Today's pending candle (crash mode on, while a miss run is active and unlogged) now specifically spans from the deepest point the crash reached so far up to `+1`, rendering as one long blue candle that visually reads as "still possible to recover" instead of just a small tick at the current depth.
  - The pending color (previously a hardcoded blue, `Color(0xFF2563EB)`) is now a themeable `pendingColor` param threaded through `InsightsScreen` → `StreakMomentumGraph`/inspector → `MomentumCandleChart`/`MomentumLineChart`. Settings > Theme gained a 5th role, **Pending candle** (`ThemeRole.PENDING_CANDLE`), with presets **Blue candle**, **Sky candle**, **Indigo candle**, plus the existing custom color picker.
  - The chevron-expanded inspector no longer auto-zooms its Y-axis to fit the whole series into a fixed 200dp-tall view. It now uses a fixed dp-per-streak-unit scale (`baseInspectorUnitHeight = 26.dp`, `baseInspectorCandleWidth = 12.dp`/`baseInspectorCandleGap = 10.dp` for candles, `baseInspectorBarWidth = 14.dp`/`baseInspectorBarGap = 6.dp` for the line chart), so the chart's content height/width grows with the value range instead of being squashed, and it pans in both directions like a stock-charting app: horizontal scroll for time (unchanged) and a new vertical scroll for value, with the Y-axis and chart scrolling together in the same `verticalScroll` container. It opens anchored at the bottom of the value range (today's baseline plus the date axis), matching the existing "anchored on now" horizontal-scroll behavior, and the user scrolls up to see higher streak peaks. A zoom in/out control (`inspectorZoom`, 50%-250% in 25% steps) sits above the chart and scales the base dp-per-unit height and candle/bar width+gap together, so zooming in enlarges both axes at once; `MomentumCandleChart` and `MomentumLineChart` now take `candleWidth`/`gap` and `barWidth`/`gap` parameters (defaulting to the prior hardcoded sizes) so the compact card's sizing is unaffected. so it always shows today pinned at the right edge and can never be left scrolled away into old history; the inspector remains the dedicated place to pan/zoom through the full timeline. The inspector's `LaunchedEffect`s that re-anchor on today (both horizontal and vertical) now also key on `inspectorZoom`, so changing the zoom level re-anchors immediately instead of only reacting to data/mode changes - the "always anchored on today" guarantee now holds in both the compact card and the inspector. The inspector's chart viewport also grew from `200.dp` to `340.dp` tall, and the `Dialog` now uses `DialogProperties(usePlatformDefaultWidth = false)` with the `Card` at `fillMaxWidth(0.96f)` instead of the platform-default (narrower) dialog width, giving the graph noticeably more room.
  - The separate `MomentumValueAxis` row (day-of-month labels below the whole chart) was removed. Dates are now drawn directly inside `MomentumCandleChart`/`MomentumLineChart`'s `Canvas` right on the zero baseline, using `rememberTextMeasurer()`/`drawText`. Each candle/point's label defaults to sitting just below the axis line; if that candle dips below zero (its `bottomValue`/value is negative), the label flips to just above the line instead, so it never overlaps the candle body or line/dot. Both charts now reserve a small `labelGutter` (16dp) above and below their plotted value range so labels have room to render even when the zero line sits at the very top or bottom of the chart; `MomentumYAxis` gained a matching `verticalInset` parameter so its tick marks stay aligned with the chart now that the chart is taller by `2 * labelGutter`.
  - The compact (non-inspect) card no longer auto-fits its y-scale to the full min/max of the data - `MomentumCandleChart`/`MomentumLineChart` gained an optional `fixedRange: IntRange?` param; when set, the chart uses that fixed min/max instead of computing it from the data, and clamps any drawn value to that range. The compact card now passes `fixedRange = -5..5`, so it always shows a fixed -5..+5 window (staying anchored on today via the existing pinned `chartScroll`) instead of squashing the whole shape whenever a big streak or crash appears; the inspector is unaffected and keeps auto-fitting/zooming as before.
  - `MomentumCandleChart`/`MomentumLineChart` gained a `showDateLabels: Boolean = true` param that gates the date-label `drawText` call. At low inspector zoom (<=100%), candles/points pack in tight enough that the date labels overlap each other, so the inspector now passes `showDateLabels = inspectorZoom > 1f` - labels disappear at 100% or below and reappear once zoomed in past 100%. The compact card doesn't set this (always shows dates, unaffected).
- Why changed:
  - User request: a "stocks crashing" gimmick for miss streaks, distinct from the flat/blocky classic miss bar, while keeping the "new streak always restarts fresh" rule from the existing momentum graph. Follow-up 1: the pending-today candle should visually communicate a chance to recover (not just track the crash depth), and its color should be themeable like the other streak-graph colors, with the theme option explicitly labeled with the word "candle". Follow-up 2: the inspector shouldn't auto-zoom to force-fit the whole range; like a real stock chart, it should use a fixed scale and let the user scroll/pan to see more. Follow-up 3: add explicit zoom in/out controls to that same inspector view. Follow-up 4: the compact card should always stay anchored on today instead of possibly being left scrolled elsewhere. Follow-up 5: the inspector should keep the same anchor-on-today guarantee (including after zooming), and its chart area should be bigger. Follow-up 6: the date labels should live on the chart's own x-axis (the zero line) instead of a separate row, flipping sides per candle to avoid overlapping it. Follow-up 7: the compact (non-inspect) card shouldn't compress its y-axis to fit the whole data range either - it should use a fixed -5..+5 window and stay anchored on the right-most (today) point. Follow-up 8: zooming out in the inspector packs candles/points close enough that their date labels start overlapping/accumulating, so labels should hide below 100% zoom and reappear above it.
- UX impact:
  - Off by default (matches existing behavior exactly). When on, the streak graph's Y-axis can extend below zero and miss runs visually "fall" further the longer they go unbroken; today's still-pending candle stretches up toward +1 to show the streak could still be saved. Settings > Theme > Pending candle lets users recolor that pending state. The inspector view (only) now feels like a pannable, zoomable stock chart instead of an auto-fit chart; very long or very spread-out streak histories may require scrolling and/or zooming out to see the full range. The compact card on the main Insights screen is no longer draggable and always shows today at the right edge; users open the inspector to browse history. The inspector is bigger and re-anchors on today when you zoom, not just when it first opens. Dates now read directly off the zero line inside the chart itself, flipping above/below per day so they stay legible instead of a fixed row underneath. The compact card's y-axis is now a fixed -5..+5 band, always anchored on today, instead of squashing to fit whatever streak/crash values are on screen. The inspector's date labels now hide themselves at 100% zoom or below (where they'd overlap) and come back once you zoom in.
- Data/model impact:
  - None (pure UI/display transform over the existing `MomentumEntry` list; no persistence changes beyond two new preferences).
- Migration notes (if any):
  - New `SharedPreferences` boolean key `momentum_crash_mode`, defaults to `false` (off) for existing installs.
  - New `SharedPreferences` keys `theme_pending_candle` (default `"blue_candle"`) and `theme_pending_candle_custom_hex` (default `#2563EB`, matching the prior hardcoded pending color) for existing installs.

## Version 1.108 (2026-08-22)
- Change summary:
  - The Insights tab's **Workout Insights** and **Progress Graphs (Beta)** cards were restyled from a title + full-width `OutlinedButton` ("Open" + chevron) to a single title row with a trailing chevron `IconButton`, matching the Streak Momentum graph's expand affordance (and the Settings `SettingsNavCard` chevron pattern).
- Why changed:
  - Consistency: all three "tap to expand/open" affordances on the Insights tab (Streak graph, Workout Insights, Progress Graphs) now look and behave the same way.
- UX impact:
  - Both cards are shorter (no separate button row) and read as a simple title-with-chevron row.
- Data/model impact:
  - None (UI-only change).
- Migration notes (if any):
  - None.
