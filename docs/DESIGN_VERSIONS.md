...
- Change summary:
  - Insights streak visualization now defaults to a new **Streak Momentum** graph (replacing the routine triangle). A pure helper `buildStreakMomentumSeries(completedDays, todayEpochDay)` emits, per run of consecutive completed days, a `0` (break/reset) then `1..runLength` (the climb), e.g. `[0,1,2,3,4, 0,1,2,3]`; it also appends a trailing `0` when the most recent run has already broken (a full day was missed before today, e.g. yesterday) so the latest break shows as a red drop — today/yesterday being unlogged is not yet a break. Rendered as a horizontally scrollable line chart (`MomentumLineChart`) that auto-anchors to the right-most (latest) entry via `LaunchedEffect(scroll.maxValue)`. The inline graph is just a title + chart; "Best streak: N" shows only inside the inspector.
